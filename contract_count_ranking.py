...
def main():
    ...
    # 日次別ファイル作成
    file1 = open('./contract_count_ranking/contract_count_ranking_' + dt.now().strftime('%Y%m%d') + '.csv', 'w', encoding="utf-8")
    writer1 = csv.writer(file1, lineterminator='\n')
    # 累積ファイル作成
    file2 = open('./data/contract_count_ranking.csv', 'a', encoding="utf-8")
